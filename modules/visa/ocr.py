...
def extract_visa_fields(image: np.ndarray) -> dict:
    ...
    return {
        ...
        "date_of_issue":    _extract_date(raw_text, ['date of issue', 'issued on', 'issue date']),
        "date_of_expiry":   _extract_date(raw_text, ['valid until', 'expiry', 'valid till', 'date of expiry']),
...
def _extract_visa_number(text: str) -> Optional[str]:
    ...
    match = re.search(r'(?:visa\s*no|visa\s*number)[:\s]+([A-Z0-9]{6,16})', text, re.IGNORECASE)
    if match:
...
def _extract_visa_type(text: str) -> Optional[str]:
    ...
    types = ['tourist', 'business', 'employment', 'student', 'transit',
             'medical', 'conference', 'journalist', 'research', 'entry']
...
def _extract_name(text: str) -> Optional[str]:
    match = re.search(r'(?:name|applicant)[:\s]+([A-Z][A-Za-z\s]{2,40})', text, re.IGNORECASE)
